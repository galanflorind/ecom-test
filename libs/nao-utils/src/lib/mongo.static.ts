...
/**
 * Create a quick search with mongo
 *
 * @deprecated
 *    > Replace with the class above
 */
const quickMongoSearchQuery = (searchTerm: string, fields: string[], currentValue: string, cvField: string, cond: '$or'|'$and' = '$or') => {
  // -->Default: options
  const defaultOptions = { cond: '$or', searchType: 'context' }
  if (!Array.isArray(fields)) {
    throw new Error(`fields must be an array`);
  }
  // -->Set: query
  const res: any = { query: {} }, query: any = {};
  let q: any = {};
  // -->Check: if there is any value the add the cond object
  if (searchTerm || currentValue) {
    res.query[cond] = [];
  }

  // -->Don't loop if searchTerm is null
  if (searchTerm) {
    // -->Loop: fields
    for (const fieldName of fields) {
      q = {};
      // -->Set: value
      q[fieldName] = {$regex : `.*${searchTerm}.*`};
      // -->Push: query
      res.query[cond].push(q);
    }
  }
  // -->Check: current value
  if (currentValue) {
    ...
    if (Array.isArray(currentValue)) {
      // -->Set: value
      // todo: must be sent as ObjectId
      q[cvField] = { $in: currentValue };
    } else {
      // -->Set: value
      q[cvField] = currentValue;
    }
    // -->Push: value
    res.query[cond].push(q);
  }
