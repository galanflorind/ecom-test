...
export class NaoUserAccessService {
  ...
  public async refreshSessionData(
    ...
    // -->Request: user login
    const sessionData = await this.naoHttp2ApiService.postJson<any>(`${this.api.root}/refresh/${naoQueryOptions.docName}/data`, { data: {}, naoQueryOptions }).toPromise();
    // -->Return
  ...
  public loginWithEmail(
    ...
    return this.naoHttp2ApiService.postJson<NaoUserAccessInterface.LoginResponse>(
      `${this.api.root}/login/${naoQueryOptions.docName}/email`, { data: { email, password }, naoQueryOptions })
  ...
  public async loginWithToken(
    ...
    // -->Reset: the session
    return this.naoHttp2ApiService.postJson(`${this.api.root}/reset/${naoQueryOptions.docName}/session`, { data: { resetToken: loginData.resetToken }, naoQueryOptions })
      .toPromise()
      .then(loginData$ => {
        if (loginData$) {
          // -->Return
  ...
  public async logout(
    ...
    // -->Logout
    return this.naoHttp2ApiService.postJson<{ ok: boolean }>(`${this.api.root}/logout/${naoQueryOptions.docName}/email`, {})
      .toPromise()
      .then(() => {
        // -->Clear: the store
        this.clearLoginData();
  ...
  public lockWithPassword(
    ...
    // -->Request: lock
    return this.naoHttp2ApiService.postJson<{ ok: boolean }>(`${this.api.root}/session/${naoQueryOptions.docName}/lock`, { data: {}, naoQueryOptions }).toPromise().then(res => {
  ...
  public unlockWithPassword(
    ...
    // -->Request: lock
    return this.naoHttp2ApiService.postJson<{ ok: boolean }>(`${this.api.root}/session/${naoQueryOptions.docName}/unlock`, { data: { password }, naoQueryOptions }).toPromise();
  }
